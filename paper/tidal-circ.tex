...
The transition occurs around a characteristic period \changes{--- the
``circularization period'' or ``transition period'' ---} that depends on the age
and evolutionary state of the population (see, e.g., \figurename~5 in
\citealt{Mathieu:2005}).
Many studies have measured this \changes{transition} period, predominantly for
...
For giant stars, the \changes{transition} period appears to be longer, closer to
...
dissipation to predict the timescale and efficiency of this phenomenon,
\changes{which set predictions for the transition period for a stellar
population with a range of masses and/or ages (see \citealt{Mazeh:2007hp,
Zahn:2008} for recent reviews).}
...
been shown to reasonably reproduce the \changes{transition} periods of a small
sample of binaries with giant star members in open clusters
...
convective and dissipative properties of the convective envelope, \changes{and $t_{\textrm{circ}}$ is the characteristic circularization timescale.}
(\citealt{Zahn:1977, Zahn:1989, Verbunt:1995}).
Following \citet{Verbunt:1995}, we set $f=1$.
\changes{Numerical simulations indicate that the observed transition period can be interpreted as the orbital period at which a binary system with initial eccentricity $e \sim 0.3$ evolves to have negligible eccentricity ($e \lesssim 0.01$) at the age of the population.
TODO: expression related to t_circ?
}

\changes{The expressions above are linearized and thus are only strictly correct for small initial eccentricities (\citealt{}).
...
While circularization is expected to be less efficient for very high-eccentricity systems (e.g., \citealt{Goldman:1991, Goodman:1997}), TODO successfully predict the upper envelope of the eccentricity distribution as a function of period even for high-eccentricity ``heartbeat'' star systems (\citealt{Shporer:2016}).
}
...
then the \changes{transition} period of a sample of binary stars should
correlate with the present-day radius or evolutionary state of the primary.

Using a sample of $>200$ binary star systems with at least one evolved member,
we study the \changes{transition} period along the giant branch.
We show that indeed the inferred \changes{transition} period is a function of
the surface gravity of the primary star.
\changes{We therefore conclude that, for binary star systems with one evolved member, orbital circularization must occur faster than the timescale of stellar radius and structure evolution of post-main-sequence stars.}
